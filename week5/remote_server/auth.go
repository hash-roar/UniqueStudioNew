...
func ParseHeader(conn *net.TCPConn) (uint8, error) {
	// +----+----------+----------+
	...
	length, _ := io.ReadFull(conn, header[:2])
	sockVersion := header[0]
	if length != 2 {
		return 0, errors.New("parse header error")
	}
	methodNum := header[1]
	length, _ = io.ReadFull(conn, header[:methodNum])
...
func AuthClient(conn *net.TCPConn) (bool, error) {
	// +----+--------+
	// |VER | METHOD |
	// +----+--------+
	// | 1  |   1    |
	// +----+--------+
	if _, err := conn.Write([]byte{SocksV5, AuthUserPass}); err != nil {
		return false, err
	}
...
func getUserInfo(conn *net.TCPConn) (version byte, name string, pass string, err error) {
	...
	if length, _ := io.ReadFull(conn, buffer[:1]); length != 1 {
	...
	nameLength := buffer[0]
	if length, _ := io.ReadFull(conn, buffer[:nameLength]); length != int(nameLength) {
	...
	passLength := buffer[0]
	if length, _ := io.ReadFull(conn, buffer[:passLength]); length != int(passLength) {
